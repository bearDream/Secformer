...
"""
    @Deprecated
    using types transpose_dimension()
"""
def transpose_multi_array(original):
    # Target transposed MultiArray
    print("original shape is: ", original.shape)
    T = sfix.Tensor([original.shape[0], original.shape[2], original.shape[1]])
    wSfix = sfix(size=original.shape[0] * original.shape[2] * original.shape[1])
    for i in range(original.shape[0]):
        print("transpose_multi_array ---------> Batch: ", i, '/', original.shape[0])
        arr = Array(length=original.shape[0] * original.shape[1] * original.shape[2], value_type=original.value_type)
        exArr = []
        for k in range(original.shape[2]):
            # print("transpose_multi_array -> foreach third dimension: ", k)
            for j in range(original.shape[1]):
                exArr.append(original[i][j][k])
        arr.assign(exArr)
        wSfix += arr
    wSfix.store_in_mem(T.get_addresses(None,None,None))
    return T

def _get_sliced_data(multiArray, size):
    # Get the sizes of the dimensions
    dim1, dim2, dim3 = multiArray.sizes

    # Create a new MultiArray with reduced second dimension size
    sliced_array = sfix.Tensor([dim1, size, dim3])

    # Copy the sliced data
    @for_range_opt([dim1, size])
    def _(i, j):
        sliced_array[i][j][:] = multiArray[i][j][:]

    return sliced_array

def store_multi_array(original):
    """
        Input: original: numpy array.
        Output: MultiArray.
    """
    # Target transposed MultiArray
    T = sfix.Tensor([original.shape[0], original.shape[1], original.shape[2]])
    wSfix = sfix(size=original.shape[0] * original.shape[2] * original.shape[1])
    for i in range(original.shape[0]):
        print("transpose_multi_array ---------> Batch: ", i, '/', original.shape[0])
        arr = Array(length=original.shape[0] * original.shape[1] * original.shape[2], value_type=T.value_type)
        exArr = [0]*original.shape[1]*original.shape[2]
...
def storeMultiArray(arr1, arr2, arr3):
    T = MultiArray([arr1.shape[0], arr2.shape[0], arr3.shape[0]], arr1.value_type)

    # print_ln("---%s---", arr1[0].reveal())
    # print_ln("---%s---", arr2[0].reveal())
    # print_ln("---%s---", arr2[1].reveal())
    # print_ln("---%s---", arr2[2].reveal())
    print_ln("---%s---", arr3[0].reveal())
    print_ln("---%s---", arr3[1].reveal())
    arr1[0].store_in_mem(T.get_addresses(*(0, 0, 0)))
    arr2[0].store_in_mem(T.get_addresses(*(0, 1, 0)))
    arr3[0].store_in_mem(T.get_addresses(*(0, 2, 0)))
    for i in range(arr1.length):
        for j in range(arr2.length):
            for k in range(arr3.length):
                print_ln("T[%s][%s][%s]:...%s...", i, j, k, T[i][j][k].reveal())
    # print_ln("T[0][0][1]:...%s...", T[0][0][1].reveal())
    # print_ln("T[0][0][2]:...%s...", T[0][0][2].reveal())
    # 遍历 pa, sa, fa 的每个元素
    # for i in range(arr1.length):
    #     for j in range(arr2.length):
    #         for k in range(arr3.length):
    #             if i == 0 and j == 0 and k == 0:
    #                 print_ln("addr1:...%s...", T.get_addresses(*(0, 0, 0)).reveal())
    #                 print_ln("addr2:...%s...", T.get_addresses(*(0, 0, 1)).reveal())
    #                 print_ln("addr3:...%s...", T.get_addresses(*(0, 0, 2)).reveal())
    #             # 获取 pa, sa, fa 的值
    #             pa_value = arr1[i]
    #             sa_value = arr2[j]
    #             fa_value = arr3[k]
    #             # print_ln("---%s---", pa_value.reveal())
    #             # 在addresses数组中找到对应的内存地址
    #             address = T.get_addresses(*(i, j, k))
    #             # 将 pa, sa, fa 的值存储在 new_multi_array 的对应内存地址中
    #             # 这里我们逐个元素存储，而不是加在一起
    #             pa_value.store_in_mem(address)
    #             sa_value.store_in_mem(address + 1)  # 假设每个值都是单个单元格
    #             fa_value.store_in_mem(address + 2)  # 根据实际情况调整偏移量
    #             print_ln("...%s...", T[i][j][k].reveal())
...
class GeLU(ElementWiseLayer):
    ...
    def f_prime(self, x):
        # GeLU derivative
        return self.phi(x) + x * self.phi_prime(x)
    ...
    def f_prime_part(self, base, size):
        x = self.Y.get_part_vector(base, size)
        return self.f_prime(x)

    def phi_prime(self, x):
        # 实现标准正态分布的概率密度函数
        # 标准正态分布的 PDF: (1 / sqrt(2 * pi)) * exp(-x^2 / 2)
        from Compiler import mpc_math
        e  = 2.718281828459045
        pi = 3.141592653589793
        two_pi = sfix(2 * pi)
        return (1 / mpc_math.sqrt(two_pi)) * mpc_math.pow_fx(e, -(sfix(0.5) * x * x))

    def phi(self, x):
        # 标准正态分布的累积分布函数
        from Compiler import mpc_math
        return 0.5 * (1 + self.erf_taylor7(x * mpc_math.InvertSqrt(sfix(2))))

    def erf_taylor2(self, x):
        a1 = sfix(0.3275911)
        a2 = sfix(-0.5)
        p = sfix(0.3275911)
    ...
    def erf_taylor5(self, x):
        ...
        t = 1 / (1 + p * x_abs)
        y = 1 - (((((a5 * t) + a4) * t + a3) * t + a2) * t + a1) * t * mpc_math.pow_fx(e, -(x_abs * x_abs))

        sign = sign.reveal()
        return sign.if_else(-y, y)

    def erf_taylor7(self, x):
        a1 = sfix(0.254829592)
        a2 = sfix(-0.284496736)
        a3 = sfix(1.421413741)
        a4 = sfix(-1.453152027)
...
class PoolBase1D(NoVariableLayer):
    def __init__(self, shape, strides=(1, 2, 1), ksize=(1, 2, 1), padding='VALID'):
        ...
        if padding == 'SAME':
            output_shape = [int(math.ceil(shape[i] / strides[i])) for i in range(3)]
            padding = [0]
        else:
            if padding == 'VALID':
                padding = 0
            if isinstance(padding, int):
                padding = [padding]
            output_shape = [shape[0], (shape[1] + 2 * padding[0] - ksize[1]) // strides[1] + 1, shape[2]]
        ...
        self.N = shape[0]
        self.comparisons = MultiArray([self.N, self.X.sizes[2], output_shape[1], ksize[1]], sint)

    def __repr__(self):
        return '%s(%s, strides=%s, ksize=%s, padding=%s)' % (
        type(self).__name__, self.X.sizes, self.strides, self.ksize, self.padding)

    def traverse(self, batch, process):
        need_padding = [self.strides[i] * (self.Y.sizes[i] - 1) + self.ksize[i] > self.X.sizes[i] for i in range(3)]

        @for_range_opt_multithread(self.n_threads, [len(batch), self.X.sizes[2]])
        def _(l, k):
            bi = batch[l]
            XX = self.X[bi]
            ...
            def _(i):
                base = self.strides[1] * i - self.padding[0]
                indices = [base + jj for jj in range(self.ksize[1])]
                ins = [(idx < self.X.sizes[1]) * (idx >= 0) for idx in indices] if need_padding[1] else [True] * \
                                                                                                        self.ksize[1]

                pool = []
                for ii, idx in enumerate(indices):
                    if not is_zero(ins[ii]):
                        pool.append([ins[ii] * XX[ins[ii] * idx][k], ins[ii], idx])
                process(pool, bi, k, i)


class MaxPool1D(PoolBase1D):
    """ Fixed-point MaxPool1d layer.

    :param shape: input shape (tuple/list of three int)
    :param strides: strides (tuple/list of four int, first and last must be 1)
    :param ksize: kernel size (tuple/list of three int, first and last must be 1)
    ...
    def backward(self, compute_nabla_X=True, batch=None):
        if compute_nabla_X:
            ...
            def process(pool, bi, k, i):
                for (x, in_, idx), c in zip(pool, self.comparisons[bi][k][i]):
                    res = in_ * c * self.nabla_Y[bi][i][k]
                    get_program().protect_memory(True)
                    self.nabla_X[bi][in_ * idx][k] += res
...
class ProbMask:
    def __init__(self, B, H, L, index, scores):
        # 1. Initialize a mask with upper triangle set to True and rest set to False
        _mask = [[True for _ in range(len(scores[0][0]))] for _ in range(L)]
        for i in range(1, len(_mask)):
            _mask[i][i:] = [False] * (len(_mask) - i)

        # 2. Expand this mask for each batch and head
        self._mask = []
        for b in range(B):
            _mask_b = []
            for h in range(H):
                # 3. Copy the mask for each index in 'index'
                _mask_h = [list(_mask_row) for _mask_row in _mask]
                for idx in index[b][h]:
                    _mask_h[idx] = _mask_h[idx]
                _mask_b.append(_mask_h)
            self._mask.append(_mask_b)
...
def custom_softmax(x, dim=-1):
    ...
    :return:
    """
    x_sm = sfix.Tensor(x.sizes)

    @library.for_range_opt([x.sizes[0], x.sizes[1], x.sizes[2]])
    def _(i, j, k):
        x_sm[i][j][k] = softmax(x[i][j][k])
    return x_sm
...
class ProbAttention:
    ...
    def _slow_prob_QK(self, Q, K, sample_k, n_top):
        B, H, L_K, E = K.shape
        _, _, L_Q, _ = Q.shape

        # 1. calculate the sampled Q_K
        K_sample = sfix.Tensor((B, H, L_Q, sample_k, E))
        Q_K_sample = sfix.Tensor((B, H, L_Q, sample_k))
        import numpy as np
        random_indices = np.random.randint(0, L_K, size=(B, H, L_Q, sample_k))
        # expand
        @library.for_range_opt([B, H, L_Q, sample_k, E])
        def _(b, h, lq, sk, e):
            K_sample[b][h][lq][sk][e] = K[b][h][cint(int(random_indices[0][0][0][0]))][e]

        # calculate Q and K sample dot
        K_sample = K_sample.transpose_5dimension()
        @library.for_range_opt([B, H, L_Q])
        def _(b, h, lq):
            Q_2d = sfix.Matrix(1, E)
            for e in range(E):
                Q_2d[0][e] = Q[b][h][lq][e]

            Q_indices = regint.inc(E, repeat=sample_k)
            K_sample_indices = regint.inc(sample_k, step=E, repeat=1)
            Q_K_sample_result = Q_2d.direct_mul(K_sample[b][h][lq], indices=(Q_indices, regint.inc(sample_k), K_sample_indices, regint.inc(E)))
            for e in range(min(E, sample_k)):
                Q_K_sample[b][h][lq][e] = Q_K_sample_result[e]


        # 2. find the Top_k query with sparisty measurement
        M = sfix.Tensor([B, H, L_Q])
        @library.for_range_opt([B, H, L_Q])
        def _(b, h, lq):
            # 计算 M
            max_value = sfix(0)
            sum_value = sfix(0)
            for e in range(sample_k):
                sum_value += Q_K_sample[b][h][lq][e]
                max_value = Q_K_sample[b][h][lq][e].max(max_value)
            M[b][h][lq] = max_value - sum_value / L_K

        M_top_index = self.__find_top_k_indices__(M, B, H, L_Q, n_top)
        Q_K = self.__calculate_Q_K__(Q, K, M_top_index, B, H, L_K, E, n_top)
        return Q_K, M_top_index
    ...
    def _prob_QK(self, Q, K, sample_k, n_top):
        ...
        K_sample = sfix.Tensor((B, H, L_Q, sample_k, E))
        Q_K_sample = sfix.Tensor((B, H, L_Q, sample_k))
        print("K_sample shape is ", K_sample.shape)
        ...
        # calculate Q and K sample dot
        K_sample = K_sample.transpose_5dimension()
        print_ln('Before Q_K_sample')
        @library.for_range_opt([B, H, L_Q])
        def _(b, h, lq):
            for e in range(E):
                temp_sum = 0
                for sk in range(sample_k):
                    # 计算 Q[b][h][lq][d] 与 K_sample_transposed[b][h][e][sk][lq] 的乘积并累加
                    temp_sum += Q[b][h][lq][e] * K_sample[b][h][lq][e][sk]
                # 将累加结果赋给 Q_K_sample
                if e < sample_k:
                    Q_K_sample[b][h][lq][e] = temp_sum
        print_ln("Q_K_sample shape is: %s", Q_K_sample.shape)
        ...
        def _(b, h, lq):
            ...
            for e in range(sample_k):
                ...
                max_value = Q_K_sample[b][h][lq][e].max(max_value)
            M[b][h][lq] = max_value - sum_value / L_K
        print_ln('M shape: %s', M.shape)
        print_ln('n_top: %s', n_top)

        M_top_index = self.__find_top_k_indices__(M, B, H, L_Q, n_top)
        print_ln('M_top_index shape: %s', M_top_index.shape)
        Q_K = self.__calculate_Q_K__(Q, K, M_top_index, B, H, L_K, E, n_top)
        print_ln('Q_K shape: %s', Q_K.shape)
        return Q_K, M_top_index

    # calculate QK
    def __calculate_Q_K__(self, Q, K, M_top, B, H, L_K, E, n_top):
    ...
    # find k indices
    def __find_top_k_indices__(self, M, B, H, L_Q, n_top):
    ...
    def _sec_get_initial_context(self, V, L_Q):
        # V MultiArray; L_Q: 64
        B, H, L_V, D = V.shape
        if not self.mask_flag:
            ...
            raise RuntimeError("not support mask_flag, please set mask_flag False...")
        return contex

    def _sec_update_context(self, context_in, V, scores, index, L_Q, attn_mask):
        """
    ...
    def forward(self, queries, keys, values, attn_mask):
        ...
        queries = queries.transpose_4dimension()
        keys = keys.transpose_4dimension()
        values = values.transpose_4dimension()
        ...
        print("sample_k: ", U_part)
        # scores_top, index = self._prob_QK(queries, keys, sample_k=U_part, n_top=u)
        scores_top, index = self._slow_prob_QK(queries, keys, sample_k=U_part, n_top=u)

        print_ln("scores_top shape: %s", scores_top.shape)
        print_ln("index shape: %s", index.shape)
        # raise RuntimeError("Debug abort...")
        ...
        #
        # context = self._sec_get_initial_context(values, L_Q)
...
class AttentionLayer:
    def __init__(self, attention, N, d_model, n_heads,
        ...
        self.query_projection = Dense(N, d_model, d_keys * n_heads)
        self.key_projection = Dense(N, d_model, d_keys * n_heads)
        self.value_projection = Dense(N, d_model, d_values * n_heads)
        self.out_projection = Dense(N, d_values * n_heads, d_model)
        self.attn_mask = None
        # Adding the nabla tensors for gradient compatibility
        self.nabla_Q = Dense(N, d_model, d_keys * n_heads)
        self.nabla_K = Dense(N, d_model, d_keys * n_heads)
        self.nabla_V = Dense(N, d_model, d_values * n_heads)
        # layers.append(self.query_projection)
        # layers.append(self.key_projection)
        # layers.append(self.value_projection)
        # layers.append(self.out_projection)

    def setter(self, queries=None, key=None, value=None):
        assert queries is not None
        assert key is not None
        assert value is not None
        self.query_projection.X = queries
        self.key_projection.X = key
        self.value_projection.X = value

    def reset(self):
        self.query_projection.reset()
        self.key_projection.reset()
        self.value_projection.reset()
        self.out_projection.reset()

    def reshape_attention_input(self, tensor):
        # Assuming B is batch size, L is sequence length and H is number of heads
        B, L, BL = tensor.shape
        _, S, _ = tensor.shape
        H = self.n_heads
        # B, L = self.N, len(tensor) // self.d_model
        return tensor.view([B, L, H, int(BL/H)])

    def _set_attn_mask(self, attn_mask):
        self.attn_mask = attn_mask

    def forward(self, q=None, k=None, v=None, batch=None):
        assert q is not None
        assert k is not None
        assert v is not None
        self.query_projection.X = q
        self.query_projection.Y = q
        self.key_projection.X = k
        self.key_projection.Y = k
        self.value_projection.X = v
        self.value_projection.Y = v

        self.query_projection.forward(batch)
        query = self.query_projection.Y
        self.key_projection.forward(batch)
        key = self.key_projection.Y
        self.value_projection.forward(batch)
        value = self.value_projection.Y

        # Reshape tensors to fit the attention mechanism
        queries = self.reshape_attention_input(query)
        keys = self.reshape_attention_input(key)
        values = self.reshape_attention_input(value)

        out, attn = self.inner_attention.forward(
            queries,
            keys,
            values,
            self.attn_mask
        ...
        # TODO
        # if self.mix:
        #     # You may need additional tensor operations to support the transpose and contiguous operations
        #     out = out.transpose(2, 1).contiguous()
        #
        # # Collapse the attention output tensor back to the original shape
        # out = out.reshape([self.N, -1])
        # self._Y = self.out_projection.forward(out)

    def backward(self, compute_nabla_X=True, batch=None):
        self.out_projection.backward(compute_nabla_X=compute_nabla_X, batch=batch)
...
class Conv1d(ConvBase):
    def n_summands(self):
        _, kernel_size, _ = self.weight_shape
        _, input_size, n_channels_in = self.input_shape
        return kernel_size * n_channels_in

    def _forward(self, batch):
        assert (self.weight_shape[0] == self.output_shape[-1])
        _, kernel_size, _ = self.weight_shape
        _, input_size, n_channels_in = self.input_shape
        _, output_size, n_channels_out = self.output_shape
        ...
        if self.use_conv2ds:
            ...
            def _(i, j):
                ...
                res = sint(size=output_size * part_size)
                # 这里调用Conv1d的卷积操作，与Conv2d代码的conv2ds类似 '6'
                conv1ds(res, inputs, weights, output_size, input_size,
                        kernel_size, stride[0], n_channels_in, padding[0],
                        part_size)
                if self.bias_before_reduction:
                    res += self.bias.expand_to_vector(j, res.size).v
                else:
                    res += self.bias.expand_to_vector(j, res.size).v << self.input_squant.f
                addresses = regint.inc(res.size, self.unreduced[i * part_size].address + j, n_channels_out)
        ...
        @for_range_opt_multithread(self.n_threads, [output_size, n_channels_out])
        def _(out_x, out_c):
            in_x_origin = out_x * stride
            iv = []
            wv = []
            for filter_x in range(kernel_size):
                in_x = in_x_origin + filter_x
                inside_x = (0 <= in_x) * (in_x < input_size)
                if is_zero(inside_x):
                    continue
                for in_c in range(n_channels_in):
                    iv += [self.X[0][in_x * inside_x][in_c]]
                    wv += [self.weights[out_c][filter_x][in_c]]
                    wv[-1] *= inside_x
            if self.fewer_rounds:
                inputs[out_x][out_c].assign(iv)
                weights[out_x][out_c].assign(wv)
            else:
                self.dot_product(iv, wv, out_x, out_c)

        if self.fewer_rounds:
            @for_range_opt_multithread(self.n_threads, [output_size])
            def _(out_x):
                self.dot_product(inputs[out_x], weights[out_x], out_x, 0)
...
class FixConv1d(Conv1d, FixBase):
    def reset(self):
        assert not self.tf_weight_format
    ...
    def backward(self, compute_nabla_X=True, batch=None):
        assert self.use_conv1ds

        assert not self.tf_weight_format
        _, weights_w, _ = self.weight_shape
        _, inputs_w, n_channels_in = self.input_shape
        _, output_w, n_channels_out = self.output_shape
        ...
        def _(i):
            self.nabla_bias.assign_vector(
                self.nabla_bias.get_vector() + self.nabla_Y[i].get_vector())

        input_size = inputs_w * N
        batch_repeat = regint.Matrix(N, inputs_w)
        ...
        def _(i, j):
            ...
            b = regint.inc(N * output_w, self.nabla_Y.address + j, n_channels_out, N)
            rep_out = regint.inc(output_w * N, 0, 1, 1, N) * \
                      reduce(operator.mul, self.output_shape[1:])
            nabla_outputs = sfix.load_mem(rep_out + b).pre_mul()
            res = sint(size=weights_w)
            conv1ds(res, inputs, nabla_outputs, weights_w, inputs_w,
                    output_w, -stride_w, N, padding_w, 1)
            reduced = unreduced_sfix._new(res).reduce_after_mul()
            self.nabla_weights.assign_vector_by_indices(reduced, j, i)

        if compute_nabla_X:
            assert self.stride == 1
            reverse_weights = MultiArray(
                [n_channels_in, weights_w, n_channels_out], sfix)
            ...
            def _(l):
                ...
                def _(k):
                    addresses = regint.inc(n_channels_out,
                                           self.weights[0][weights_w - k - 1].get_address(l),
                                           reduce(operator.mul, self.weights.sizes[1:]))
                    reverse_weights[l][weights_w - k - 1].assign_vector(
                        self.weights.value_type.load_mem(addresses))

            padded_w = inputs_w + 2 * padding_w
            if padding_w:
                output = MultiArray(
                    [N, padded_w, n_channels_in], sfix)
            else:
                output = self.nabla_X

            @for_range_opt_multithread(self.n_threads, [N, n_channels_in])
            def _(i, j):
                res = sint(size=padded_w)
                conv1ds(res, self.nabla_Y[i].get_vector().pre_mul(),
                        reverse_weights[j].get_vector().pre_mul(),
                        padded_w, output_w, 1, -stride_w, N, padding_w, 1)
            ...
            if padding_w:
                ...
                def _(i):
                    ...
                    def _(k):
                        kk = k + padding_w
                        self.nabla_X[i][k].assign_vector(
                            output[i][kk].get_vector())
        ...
        if self.debug_output:
            ...
            def _(i):
                ...
                if compute_nabla_X:
                    print_ln('%s nabla X %s %s', self, i, list(self.nabla_X[batch[i]].reveal_nested()))
...
def easyConv1d(input_shape, batch_size, out_channels, kernel_size, stride=1,
               padding=0):
    """ More convenient interface to :py:class:`FixConv1d`.

    :param input_shape: input shape (tuple/list of three int)
    :param out_channels: output channels (int)
    :param kernel_size: kernel size (int)
    :param stride: stride (int)
    ...
    output_shape = [batch_size] + list(
        apply_padding1d(input_shape[2:], kernel_size, stride, padding)) + \
                   [out_channels]
    padding = padding.upper() if isinstance(padding, str) else padding
    return FixConv1d(input_shape, weight_shape, (out_channels,), output_shape,
                     stride, padding)
...
class TransformOptimizer:
    ...
    def forward(self, N=None, batch=None, keep_intermediate=True,
        ...
        for i, layer in enumerate(self.layers):
            ...
            if self.time_layers:
                start_timer(100 + i)
            if i != len(self.layers) - 1 or run_last:
                # Attention layer and other layers forward.
                # For Encoder, (x_enc, x_mark_enc) required
                # For Decoder, (enc_out, dec_out) required
    ...
    def run(self, batch_size=None, stop_on_loss=0):
        ...
        if batch_size is not None:
            ...
            N = self.layers[0].X.size(0)  # 适配Informer的数据格式
        ...
        @for_range(self.n_epochs)
        def _(_):
            # 1. 初始化随机索引（略过标签相关内容，因为Informer是回归任务）
            indices = regint.Array(N)
            indices.assign(regint.inc( self.layers[0].X.size(0)))
            if self.shuffle:
                indices.shuffle()
            ...
            # 根据Informer的数据切片方式进行批处理
            n_per_epoch = int(math.ceil(self.layers[0].X.size(0) / N))

            @for_range(n_per_epoch)
            def _(j):
                n_iterations.iadd(1)
                batch = regint.Array(N)
                indices_vector = indices.get_vector(j * N, N)
                batch.assign(indices_vector)
            ...
            self.missing_newline = False
            if self.report_loss:
                print_ln('loss in epoch %s: %s', i, (loss_sum.reveal() * cfix(1 / n_per_epoch)))
            i.iadd(1)
            res = True
            if self.tol > 0:
                res *= (1 - (loss_sum >= 0) * (loss_sum < self.tol * n_per_epoch)).reveal()
            self.stopped_on_low_loss.write(1 - res)
            return res


    @_no_mem_warnings
    def run_by_args(self, program, n_runs, batch_size,
                    test_batch_x, test_batch_x_mark, test_dec_inp, test_batch_y_mark,
                    acc_batch_size=None, reset=True):
        MultiArray.disable_index_checks()
        Array.check_indices = False
        if acc_batch_size is None:
    ...
    def fit(self, x_enc, x_mark_enc, x_dec, x_mark_dec,
            epochs=1, batch_size=128, validation_data=(None, None, None, None),
            program=None, reset=True, print_accuracy=False, print_loss=False):
        """ Train model for time series data with Informer.

        :param x_enc: encoder input data (sfix tensor)
        :param x_mark_enc: encoder time series mask (sfix tensor)
        :param x_dec: decoder input data (sfix tensor)
        :param x_mark_dec: decoder time series mask (sfix tensor)
        :param epochs: number of epochs (int)
        :param batch_size: batch size (int)
        :param validation_data: tuple of validation encoder input, decoder input, target labels, and their masks
        :param program: :py:class:`~Compile.program.Program` instance to use command-line parameters (optional)
        :param reset: whether to initialize model
        :param print_accuracy: print accuracy on training data (reveals labels)
        :param print_loss: reveal and print training loss after every batch

        """
        # Embedding params in here. We need computing enc_out & dec_out using parameters below.
        # self.x_enc = x_enc
        # self.x_mark_enc = x_mark_enc
        # self.x_dec = x_dec
        # self.x_mark_dec = x_mark_dec
        # Assuming the layers are properly set up for Informer architecture
        # Set the model's input data
        self.layers[0].X = x_enc
        self.layers[0].X_mark = x_mark_enc
        self.layers[-1].Dec_inp = x_dec
        self.layers[-1].Y_mark = x_mark_dec
...
class TransformAdam(TransformOptimizer):
    """ Adam/AMSgrad optimizer for Informer.
    ...
    def __init__(self, layers, n_epochs=1, approx=False, amsgrad=False,
                 normalize=False):
        super(TransformAdam, self).__init__()
...
def apply_padding1d(input_shape, kernel_size, stride, padding):
    if isinstance(padding, int):
        padding_v = padding
        padding = [padding]
    if isinstance(padding, (tuple, list)):
        ...
        padding = 'valid'
    if padding.lower() == 'valid':
        res = ((input_shape[0] + 2 * padding_v - kernel_size[0]) // stride[0]) + 1
        assert res > 0, (input_shape, kernel_size, stride, padding)
        return [res]
    elif padding.lower() == 'same':
        return [(input_shape[0]) // stride[0]]
    else:
        raise Exception('invalid padding: %s' % padding)
...
def attn_to_layers(attn, input_shape):
    """ Convert a attn object to MP-SPDZ layers.
...
def transforms_from_torch(informer, data_input_shape, batch_size, input_via=None,
                          regression=False):
    """ Convert a PyTorch Transform object to MP-SPDZ layers.

    :param informer: PyTorch Informer(Encoder & Decoder) object
    ...
    # Encode
    #   EncoderLayer:
    #       attention(query_projection, key_projection, value_projection, out_projection)
    #       conv1
    #       conv2
    #       norm1
    #       norm2
    ...
    # Decode
    #   DecoderLayer:
    #       self_attention: attention(query_projection, key_projection, value_projection, out_projection)
    #       cross_attention: attention(query_projection, key_projection, value_projection, out_projection)
    #       conv1
    #       conv2
    #       norm1
    #       norm2
    ...
    def process(item):
        ...
        for attn_layer in encoder.attn_layers:
            attn = attn_layer.attention
            sec_attn = AttentionLayer(
                ProbAttention(False, attn.inner_attention.factor, attention_dropout=attn.inner_attention.dropout,
                              output_attention=attn.inner_attention.output_attention),
                512, attn.n_heads, mix=False)
            layers.append(sec_attn)
            conv1 = attn_layer.conv1
            sec_conv1 = easyConv1d(input_shape=input_shape, batch_size=batch_size, out_channels=conv1.out_channels,
                                   kernel_size=conv1.kernel_size[0], stride=conv1.stride[0], padding=conv1.padding[0])
            layers.append(sec_conv1)
            conv2 = attn_layer.conv2
            sec_conv2 = easyConv1d(input_shape=input_shape, batch_size=batch_size, out_channels=conv2.out_channels,
                                   kernel_size=conv2.kernel_size[0], stride=conv2.stride[0], padding=conv2.padding[0])
            layers.append(sec_conv2)
            norm1 = attn_layer.norm1
            sec_norm1 = BatchNorm((1, norm1.normalized_shape[0]))
            layers.append(sec_norm1)
            norm2 = attn_layer.norm2
            sec_norm2 = BatchNorm((1, norm2.normalized_shape[0]))
            layers.append(sec_norm2)
            dropout = attn_layer.dropout
            sec_dropout = Dropout(N=input_shape[0], d1=norm1.normalized_shape[0], alpha=dropout.p)
            layers.append(sec_dropout)
        # Encode -> conv_layers
        for conv_layer in encoder.conv_layers:
            conv1 = conv_layer.downConv
            sec_conv1 = easyConv1d(input_shape=input_shape, batch_size=batch_size, out_channels=conv1.out_channels,
                                   kernel_size=conv1.kernel_size[0], stride=conv1.stride[0], padding=conv1.padding[0])
            layers.append(sec_conv1)
            norm = conv_layer.norm
            sec_norm = BatchNorm((1, 1, norm.num_features))
            layers.append(sec_norm)
            activation = conv_layer.activation
            layers.append(GeLU(input_shape))
            maxPool = conv_layer.maxPool
            sec_maxpool = MaxPool1D(shape=input_shape, strides=maxPool.stride, ksize=maxPool.kernel_size,
                                    padding=maxPool.padding)
            layers.append(sec_maxpool)
        # Encode -> LayerNorm
        encoder_norm = encoder.norm
        sec_encoder_norm = BatchNorm((1, 1, encoder_norm.normalized_shape[0]))
        layers.append(sec_encoder_norm)

        # Decode...

        # Projection
        projection = Dense(N=input_shape[0], d_in=512, d_out=7)
